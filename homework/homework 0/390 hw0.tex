...
% \tun{def func \tmo{someExampleFunction$(arg1, arg2 := 1, arg3 := 2)$:}},
...
\begin{quote}%
\noindent\tun{def func \tmo{computeRotationIndex($A[1..n]$):}}%
...
\begin{quote}%
\noindent\tun{def func \tmo{findPeak($A[1..n]$):}}%
...
\begin{quote}%
\noindent\tun{def func \tmo{findLocalMinimum($A[1..n]$):}}%
...
\begin{quote}
\noindent\tun{def func \tmo{cyclic-Hanoi($n$, $from$, $to$):}}%
...
\begin{solution}\ % SOLUTION 2.1.3
  ...
  \begin{quote}
  \noindent\tun{def func \tmo{double-cyclic-Hanoi($n$, $from$, $to$, $spare$):}}%
...
\begin{quote}
\noindent\tun{\tmo{def func thick-Hanoi($ n $, $ A $, $ B $, $ C $):}}%
...
\begin{quote}
\noindent\tun{def func \tmo{triple-Hanoi($n$, $A$, $B$, $C$):}}%
...
\begin{solution}\ %
\begin{quote}
\noindent\tun{def func \tmo{American-Hanoi($n$, $start$):}}%
...
\nf{} \\
\begin{quote}
\noindent\tun{def func \tmo{Towers-of-Hanoi($k$, $A$, $B$, $C$):}}%
...
\begin{quote}%
\noindent\tun{def func \tmo{max-matching($G = (V, E)$):}}%
...
\begin{quote}%
  \noindent\tun{def func \tmo{max-independent-set($G = (V, E)$):}}%
...
\begin{quote}%
\noindent\tun{def func \tmo{longest-increasing-subsequence-including-first($x_1, \ldots, x_n$):}} \rcomm{LISIF for short}%
...
\begin{quote}%
\noindent\tun{def func \tmo{reachable($G = (V, E)$, $ s $, $ t $):}}%
...
\begin{quote}%
\noindent\tun{def func \tmo{partition($G = (V, E)$):}}%

\doc{Given $ n $ integers $ x_1,\ldots,x_n\in\bbZ $, returns \boolT~if there is a subset of indicies $ S\sseq [n] $ such that $ \sum_{i\in S}x_i = \sum_{i\in[n]\setminus S} x_i $, and \boolF~otherwise.}%
