...
\fancyhead[C]{\tbo{CS 390ATA: Advanced Topics in Algorithms\\Homework 3}}
\fancyhead[R]{\tbo{Spring 2025 \\ Page \thepage}}

\begin{document}
% >>==========|| 6.5 ||==========<<
\tbo{Exercise 6.5.} Let $G = (V, E)$ be a directed graph. We say two vertices $s, t \in V$ are \tit{half-connected} if either $s$ can reach $t$ or $t$ can reach $s$.
...
% >>==========|| 7.1 ||==========<<
\tbo{Exercise 7.1.} Each of the following problems describes a graph problem over an unweighted directed graph $G = (V, E)$ with $m$ edges and $n$ vertices.
...
\begin{enumerate}[label=(\alph*)]
  ...
  \begin{steps}
    ...
    \item Find the smallest $ d = \nf{dist}((s,0,0),(t,r_5,r_8)) $ such that $ d = 5x + 8y $ for some $ x, y \in \bbN $.
...
\begin{solution}
\defalgo{k-speeding}{G=(V,E),\ell:E\to\bbR_>0,s\in V,t\in V,k\in\bbN}
\begin{steps}
